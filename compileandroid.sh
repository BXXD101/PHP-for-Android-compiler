...
#cd libzip-1.9.2
#cmake . -DBUILD_SHARED_LIBS=OFF -DCMAKE_PREFIX_PATH=$zip -DCMAKE_INSTALL_PREFIX=$zip -DCMAKE_INSTALL_LIBDIR=lib -DBUILD_TOOLS=OFF -DBUILD_REGRESS=OFF -DBUILD_EXAMPLES=OFF -DBUILD_DOC=OFF -DENABLE_BZIP2=OFF -DENABLE_COMMONCRYPTO=OFF -DENABLE_GNUTLS=OFF -DENABLE_MBEDTLS=OFF -DENABLE_LZMA=OFF
#make -j$(nproc)
#make install
#cd ..
...
cd libxml2-v2.10.1
./autogen.sh --enable-static=yes --prefix=$xml --host=$TARGET --without-lzma --without-iconv --without-python --enable-shared=no --with-zlib=$lib
make -j$(nproc)
make install
cd ..
